...
const transportLabels={starter:document.getElementById('starterTransport'),comfort:document.getElementById('comfortTransport'),premium:document.getElementById('premiumTransport')};
...
const expenditureQuickButton=document.getElementById('expenditureQuickButton');
const pauseExpenditureButton=document.getElementById('pauseExpenditureButton');
...
const completionPulse=document.getElementById('completionPulse');
...
const SESSION_KEY='knowE_simulation_session';
...
let state={
    month:1,currentScene:'loading',completedMonth:0,paused:false,sessionId:'',expenditureIntroduced:false,
...
function simulationSessionId(){const id=`${Date.now()}_${Math.random().toString(36).slice(2)}`;sessionStorage.setItem(SESSION_KEY,id);return id}
state.sessionId=simulationSessionId();
...
function loadLocal(){try{const saved=JSON.parse(localStorage.getItem(userKey())||'null');if(saved&&saved.month===1&&saved.sessionId===state.sessionId){state={...state,...saved};state.transactions=(saved.transactions||[]).filter(t=>t.sessionId===state.sessionId);state.goals=saved.goals||[];state.decisions=saved.decisions||{};return true}}catch{}return false}
...
async function showNotification({app="KNOW'E LEDGER",title,message,duration=2200}){const n=document.createElement('div');n.className='floating-notification';n.innerHTML=`<div class="floating-notification-topline"><div class="floating-app-icon">⌂</div><span>${app}</span><small>NOW</small></div>${title?`<strong>${title}</strong>`:''}${message?`<p>${message}</p>`:''}`;notificationLayer.appendChild(n);requestAnimationFrame(()=>n.classList.add('visible'));await wait(Math.max(duration,3000));n.classList.remove('visible');await wait(450);n.remove()}
async function showCutscene(text,duration){cutsceneText.textContent=text;cutsceneOverlay.classList.add('visible');cutsceneOverlay.setAttribute('aria-hidden','false');await wait(duration+1500);cutsceneOverlay.classList.remove('visible');cutsceneOverlay.setAttribute('aria-hidden','true');await wait(300)}

function addTransaction(category,amount,description,type='expense'){state.transactions.push({month:state.month,sessionId:state.sessionId,category,amount:Number(amount),description,type,at:new Date().toISOString()});if(type==='expense')state.balance-=Number(amount);else state.balance+=Number(amount);updateBalance();saveLocal()}
async function financialAction({category,amount,description,type='expense',notificationTitle,notificationMessage}){addTransaction(category,amount,description,type);const message=typeof notificationMessage==='function'?notificationMessage():notificationMessage||`${type==='expense'?money(amount)+' deducted':money(amount)+' credited'}`;await showNotification({title:notificationTitle||description,message,duration:3000});await checkpointSave()}
...
function applyHousing(base){const vals={starter:Math.round(base),comfort:Math.round(base*1.3),premium:Math.round(base*.5)};document.getElementById('starterRent').textContent=`${money(vals.starter)} / MONTH`;document.getElementById('comfortRent').textContent=`${money(vals.comfort)} / MONTH`;document.getElementById('premiumRent').textContent=`${money(vals.premium)} / MONTH`;window.homeRentData=vals;renderHomeTransport()}
function renderHomeTransport(){const baseDaily=state.transport.monthlyBase/30;const factors={starter:1,comfort:.7,premium:2.2};Object.entries(factors).forEach(([home,factor])=>{if(transportLabels[home])transportLabels[home].textContent=`TRANSPORT: ${money(baseDaily*factor)} / WORKING DAY`})}
function calculateTransport(){const baseDaily=state.transport.monthlyBase/30;const home=state.selectedHome;const factor=home==='comfort'?.7:home==='premium'?2.2:1;state.transport.dailyBase=baseDaily;state.transport.daily=baseDaily*factor;state.transport.total=Math.round(state.transport.daily*CHECKPOINT_DAYS);renderHomeTransport()}
...
function activateSlide(i){const slides=document.querySelectorAll('.home-slide');const navs=document.querySelectorAll('.slide-nav');const index=(i+slides.length)%slides.length;slides.forEach((s,n)=>s.classList.toggle('active',n===index));navs.forEach((b,n)=>b.classList.toggle('active',n===index))}

document.querySelectorAll('.slide-nav').forEach(b=>b.addEventListener('click',()=>activateSlide(Number(b.dataset.slide))));
document.querySelector('.slide-prev')?.addEventListener('click',()=>{const active=Array.from(document.querySelectorAll('.home-slide')).findIndex(slide=>slide.classList.contains('active'));activateSlide(active-1)});
document.querySelector('.slide-next')?.addEventListener('click',()=>{const active=Array.from(document.querySelectorAll('.home-slide')).findIndex(slide=>slide.classList.contains('active'));activateSlide(active+1)});
...
async function confirmPhoneChoice(choice){closeDecision();currentDecisionBack=null;await hideImageScene();if(choice==='save'){if(!state.goals.find(g=>g.name==='PHONE'))state.goals.push({name:'PHONE',target:PHONE_PRICE,saved:0});await showNotification({title:'GOAL CREATED',message:'PHONE added to Goals & Savings: ₹30,000 target.'})}else await showNotification({title:choice==='delay'?'PURCHASE DELAYED':'PURCHASE SKIPPED',message:'No money was deducted.'});saveLocal();await checkpointSave();await afterPhoneChoice()}
async function afterPhoneChoice(){await showNotification({title:'FINANCIAL STATE UPDATED',message:`Money in hand: ${money(state.balance)}`});await hideImageScene();await finishMonth()}

async function finishMonth(){state.completedMonth=1;state.currentScene='month1-complete';expenditureQuickButton?.classList.add('hidden');saveLocal();await checkpointSave();monthEndOverlay.classList.add('visible');monthEndOverlay.setAttribute('aria-hidden','false');await wait(TIMES.monthEnd);monthEndCard?.classList.add('hidden');simulation.classList.add('salary-screen');const salary=Number(userData?.profile?.salary||0);if(salary>0){addTransaction('SALARY',salary,'Monthly salary','income');await showNotification({app:'KNOW’E LEDGER',title:'PAYMENT RECEIVED',message:`SALARY: ${money(salary)} credited to your account.`});await showNotification({app:'YOUR BANK',title:`${money(salary)} CREDITED`,message:`Your salary has been received.`})}dashboardButton.classList.add('visible')}
